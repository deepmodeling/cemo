...
def save_lmdb(f_output: str, f_mrcs: str):
    ...
    with mrcfile.mmap(f_mrcs) as mrc:
        ...
        mrc.print_header()
        for ind, image in enumerate(mrc.data):
            txn_writer.put(str(ind).encode("ascii"), image.dumps())

            # commit on each 10000 images
            if ind % 10000 == 9999:
                txn_writer.commit()
                print(f"Processed {ind + 1} projection images.")

    txn_writer.commit()
    print(f"Totally processed {ind + 1} projection images.")
...
def parse_args():
    ...
    p.add_argument("-i", "--input", required=True, help="input mrcs file")
    p.add_argument("-o", "--output", required=True, help="output lmdb file")
    return p.parse_args()


def main(args: Namespace):
    save_mrcs_to_lmdb(args.output, args.input)
    print(args.output)
