...
def test():
    ...
    print(f"points initial postion")
    pprint(points)
    print(torch.mean(points, dim=0))
    ...
    R = torch.tensor(Rotation.from_euler(convention, angles, degrees=False).as_matrix(),
        dtype=dtype_all)

    R = R.transpose(0, 1)
    print("Rotation matrix")
    pprint(R.round(decimals=2))
    ...
    rotmat_for_affine_grid = R @ R_correction
    ...
    print(f"projection diff\n{(projection - projection2).numpy().round(4)}")
