"""
Test apply_ctf using a reference procedure.
...
def test(L: int, is_rfft: bool, symm: bool, domain: str, apix: float):
    ...
    rfft_dim = 1 if is_rfft else None
    freq2d_enforced = dft.enforce_freq_symm(
        freq2d,
        dims=(0, 1),
        sdims=(0, 1),
        symm=symm,
    ...
    ctf = calc_ctf(freq2d_enforced, params=ctf_params, apix=apix)
    if domain == "real":
        images_ft = ctf * dft.fftn(
            images,
            dims=dims, symm_freq=symm, symm_real=True, use_rfft=is_rfft)
        expect = dft.ifftn(
            images_ft,
            dims=dims, symm_freq=symm, symm_real=True, use_rfft=is_rfft).real
    else:
        expect = ctf * images
